...
lemma hε_qo : 0 ≤ alg.ε_qo ∧ alg.ε_qo < alg.ε_qos := alg.hε_qo'
...
lemma ε_qo_lt_est_consts :
  ...
  rcases @Real.add_neg_lt_sSup
    (Set.range fun δ ↦ sSup (Set.range fun (h:δ > 0) ↦ (1 - (1 + δ) * (1 - (1 - alg.ρ_red) * alg.θ)) / (alg.C_rel ^ 2 * (alg.C_red + (1 + δ⁻¹) * alg.C_stab ^ 2))))
    (by apply Set.range_nonempty)
    (alg.ε_qo - alg.ε_qos)
    (sub_neg.mpr alg.hε_qo.2) with ⟨a, ha⟩
  ...
  have key : alg.ε_qo < (1 - alg.ρ_est δ) / (alg.C_rel^2 * alg.C_est δ) := by
    ...
    have := calc a
        _ = sSup (Set.range fun h ↦ (1 - (1 + δ) * (1 - (1 - alg.ρ_red) * alg.θ)) / (alg.C_rel ^ 2 * (alg.C_red + (1 + δ⁻¹) * alg.C_stab ^ 2))) := by rw [hδ]
  ...
  · have : 0 < 1 - alg.ρ_est δ := by
      refine (div_pos_iff_of_pos_right ?_).mp <|
        calc 0
